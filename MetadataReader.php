...
class Mo_o365_IDPMetadataReader{
	...
	public function __construct(DOMNode $xml = NULL){
		...
		$entityDescriptors = Mo_o365_Utilities::xpQuery($xml, './saml_metadata:EntityDescriptor');

		

		foreach ($entityDescriptors as $entityDescriptor) {
			$idpSSODescriptor = Mo_o365_Utilities::xpQuery($entityDescriptor, './saml_metadata:IDPSSODescriptor');
			
			if(isset($idpSSODescriptor) && !empty($idpSSODescriptor)){
				array_push($this->identityProviders,new Mo_o365_IdentityProviders($entityDescriptor));
			}
...
class Mo_o365_IdentityProviders{
	...
	public function __construct(DOMElement $xml = NULL){
        ...
        $idpSSODescriptor = Mo_o365_Utilities::xpQuery($xml, './saml_metadata:IDPSSODescriptor');
        ...
        $info = Mo_o365_Utilities::xpQuery($xml, './saml_metadata:Extensions');
	...
	private function parseInfo($xml){
		$displayNames = Mo_o365_Utilities::xpQuery($xml, './mdui:UIInfo/mdui:DisplayName');
	...
	private function parseSSOService($xml){
		$ssoServices = Mo_o365_Utilities::xpQuery($xml, './saml_metadata:SingleSignOnService');
	...
	private function parseSLOService($xml){
		$sloServices = Mo_o365_Utilities::xpQuery($xml, './saml_metadata:SingleLogoutService');
	...
	private function parsex509Certificate($xml){
		foreach ( Mo_o365_Utilities::xpQuery($xml, './saml_metadata:KeyDescriptor') as $KeyDescriptorNode ) {
	...
	private function parseSigningCertificate($xml){
		$certNode = Mo_o365_Utilities::xpQuery($xml, './ds:KeyInfo/ds:X509Data/ds:X509Certificate');
		$certData = trim($certNode[0]->textContent);
		$certData = str_replace(array ( "\r", "\n", "\t", ' '), '', $certData);
		if(!empty($certNode))
			array_push($this->signingCertificate, Mo_o365_Utilities::sanitize_certificate( $certData ));
	}
