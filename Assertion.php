...
class Mo_o365_Assertion
{
    ...
    {
        $this->id = Mo_o365_Utilities::generateId();
        $this->issueInstant = Mo_o365_Utilities::generateTimestamp();
        $this->issuer = '';
        $this->authnInstant = Mo_o365_Utilities::generateTimestamp();
        ...
        $this->issueInstant = Mo_o365_Utilities::xsDateTimeToTimestamp($xml->getAttribute('IssueInstant'));

        $issuer = Mo_o365_Utilities::xpQuery($xml, './saml_assertion:Issuer');
        if (empty($issuer)) {
    ...
    public function Return_base(){
        $url=get_site_url();
        Mo_o365_Utilities::mo_o365_saml_wp_remote_get($url);
        exit;
    }
    /**
    ...
    {
        $subject = Mo_o365_Utilities::xpQuery($xml, './saml_assertion:Subject');
        if (empty($subject)) {
        ...
        $nameId = Mo_o365_Utilities::xpQuery(
            $subject,
        ...
        if ($nameId->localName === 'EncryptedData') {
            ...
            $this->nameId = Mo_o365_Utilities::parseNameId($nameId);
        }
    ...
    {
        $conditions = Mo_o365_Utilities::xpQuery($xml, './saml_assertion:Conditions');
        if (empty($conditions)) {
        ...
        if ($conditions->hasAttribute('NotBefore')) {
            $notBefore = Mo_o365_Utilities::xsDateTimeToTimestamp($conditions->getAttribute('NotBefore'));
            if ($this->notBefore === NULL || $this->notBefore < $notBefore) {
                $this->notBefore = $notBefore;
            }
        }
        if ($conditions->hasAttribute('NotOnOrAfter')) {
            $notOnOrAfter = Mo_o365_Utilities::xsDateTimeToTimestamp($conditions->getAttribute('NotOnOrAfter'));
        ...
        for ($node = $conditions->firstChild; $node !== NULL; $node = $node->nextSibling) {
            ...
            switch ($node->localName) {
                case 'AudienceRestriction':
                    $audiences = Mo_o365_Utilities::extractStrings($node, 'urn:oasis:names:tc:SAML:2.0:assertion', 'Audience');
    ...
    {
        $authnStatements = Mo_o365_Utilities::xpQuery($xml, './saml_assertion:AuthnStatement');
        ...
        $this->authnInstant = Mo_o365_Utilities::xsDateTimeToTimestamp($authnStatement->getAttribute('AuthnInstant'));

        if ($authnStatement->hasAttribute('SessionNotOnOrAfter')) {
            $this->sessionNotOnOrAfter = Mo_o365_Utilities::xsDateTimeToTimestamp($authnStatement->getAttribute('SessionNotOnOrAfter'));
        }
    ...
    private function parseAuthnContext(DOMElement $authnStatementEl)
    {
        ...
        $authnContexts = Mo_o365_Utilities::xpQuery($authnStatementEl, './saml_assertion:AuthnContext');
        ...
        $authnContextDeclRefs = Mo_o365_Utilities::xpQuery($authnContextEl, './saml_assertion:AuthnContextDeclRef');
        ...
        $authnContextDecls = Mo_o365_Utilities::xpQuery($authnContextEl, './saml_assertion:AuthnContextDecl');
        ...
        $authnContextClassRefs = Mo_o365_Utilities::xpQuery($authnContextEl, './saml_assertion:AuthnContextClassRef');
        ...
        $this->AuthenticatingAuthority = Mo_o365_Utilities::extractStrings(
            $authnContextEl,
    ...
    {
        $firstAttribute = TRUE;
        $attributes = Mo_o365_Utilities::xpQuery($xml, './saml_assertion:AttributeStatement/saml_assertion:Attribute');
        foreach ($attributes as $attribute) {
            ...
            $values = Mo_o365_Utilities::xpQuery($attribute, './saml_assertion:AttributeValue');
    ...
    {
        $this->encryptedAttribute = Mo_o365_Utilities::xpQuery(
            $xml,
    ...
    public function validate(XMLSecurityKey $key)
    {
        ...
        Mo_o365_Utilities::validateSignature($this->signatureData, $key);
     ...
     *
     * The returned NameId is in the format used by Mo_o365_Utilities::addNameId().
     *
     * @see Mo_o365_Utilities::addNameId()
     ...
     *
     * The NameId must be in the format accepted by Mo_o365_Utilities::addNameId().
     *
     * @see Mo_o365_Utilities::addNameId()
    ...
    public function encryptNameId(XMLSecurityKey $key)
    {
        ...
        Mo_o365_Utilities::addNameId($root, $this->nameId);
        $nameId = $root->firstChild;

        Mo_o365_Utilities::getContainer()->debugMessage($nameId, 'encrypt');
    ...
    public function decryptNameId(XMLSecurityKey $key, array $blacklist = array())
    {
        ...
        $nameId = Mo_o365_Utilities::decryptElement($this->encryptedNameId, $key, $blacklist);
        Mo_o365_Utilities::getContainer()->debugMessage($nameId, 'decrypt');
        $this->nameId = Mo_o365_Utilities::parseNameId($nameId);
    ...
    public function decryptAttributes(XMLSecurityKey $key, array $blacklist = array())
    {
        ...
        foreach ($attributes as $attributeEnc) {
            ...
            $attribute = Mo_o365_Utilities::decryptElement(
    ...
    public function toXML(DOMNode $parentElement = NULL)
    {
        ...
        $issuer = Mo_o365_Utilities::addString($root, 'urn:oasis:names:tc:SAML:2.0:assertion', 'saml:Issuer', $this->issuer);
        ...
        if ($this->signatureKey !== NULL) {
            Mo_o365_Utilities::insertSignature($this->signatureKey, $this->certificates, $root, $issuer->nextSibling);
        }
    ...
    private function addSubject(DOMElement $root)
    {
        ...
        if ($this->encryptedNameId === NULL) {
            Mo_o365_Utilities::addNameId($subject, $this->nameId);
        } else {
    ...
    private function addConditions(DOMElement $root)
    {
        ...
        if ($this->validAudiences !== NULL) {
            ...
            Mo_o365_Utilities::addStrings($ar, 'urn:oasis:names:tc:SAML:2.0:assertion', 'saml:Audience', FALSE, $this->validAudiences);
        }
    }
