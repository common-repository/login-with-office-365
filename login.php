<?php
/*
Plugin Name: Azure / Office 365 SSO Login
Plugin URI: http://plugins.miniorange.com/
Description: Azure AD, Azure B2C, Office 365 Login allows Azure users to securely and safely login into WordPress with their Office 365 / Azure AD / Azure B2C accounts.
Version: 1.0.4
Author: miniOrange
...
class Mo_o365_mo_login {

	function __construct() {
		add_action( 'admin_menu', array( $this, 'mo_o365_sso_menu' ) );
		add_action( 'admin_init', array( $this, 'miniorange_login_widget_saml_save_settings' ) );
		add_action( 'admin_enqueue_scripts', array( $this, 'plugin_settings_style' ) );
		register_deactivation_hook( __FILE__, array( $this, 'mo_o365_saml_deactivate') );
		
		add_action( 'admin_enqueue_scripts', array( $this, 'plugin_settings_script' ) );
		
		remove_action( 'admin_notices', array( $this, 'mo_o365_saml_success_message' ) );
		remove_action( 'admin_notices', array( $this, 'mo_o365_saml_error_message' ) );
		add_action( 'wp_authenticate', array( $this, 'mo_o365_saml_authenticate' ) );
		add_action( 'wp', array( $this, 'mo_o365_saml_auto_redirect' ) );
		add_action( 'login_form', array( $this, 'mo_o365_saml_modify_login_form' ) );
		add_shortcode( 'MO_SAML_FORM', array( $this, 'mo_o365_get_saml_shortcode' ) );
		add_action( 'admin_footer', array( $this, 'feedback_request' ) );
        add_action( 'plugin_action_links_' . plugin_basename( __FILE__ ), array($this,'mo_o365_saml_plugin_action_links') );
	...
	function feedback_request() {
		...
		mo_o365_display_saml_feedback_form();
	}

	function mo_o365_login_widget_saml_options() {
		global $wpdb;

		mo_o365_register_saml_sso();
	}

	function mo_o365_saml_success_message() {
		$class   = "error";
		$message = get_option( 'mo_saml_message' );
		echo "<div class='" . $class . "'> <p>" . $message . "</p></div>";
	}

	function mo_o365_saml_error_message() {
		$class   = "updated";
		$message = get_option( 'mo_saml_message' );
		echo "<div class='" . $class . "'> <p>" . $message . "</p></div>";
	}

	public function mo_o365_saml_deactivate(){

        if(mo_o365_saml_is_customer_registered_saml(false))
            return;
        if(!mo_o365_saml_is_curl_installed())
            return;
        $customer = new Mo_o365_Customersaml();
	...
	public function mo_o365_saml_remove_account() {
		if ( ! is_multisite() ) {
            ...
            delete_option('mo_o365_is_new_user');
	...
	function plugin_settings_style( $page) {
		if ( $page != 'toplevel_page_mo_o365_saml_settings' && !(isset($_REQUEST['page']) && $_REQUEST['page'] == 'mo_saml_licensing')) {
			return;
		}
        ...
        $manager = new Mo_o365_PointersManager( $file, '4.8.52', 'custom_admin_pointers' );
        $manager->parse();
	...
	public function plugin_activate(){
		if(is_multisite()){
			...
			foreach($blog_ids as $blog_id){
				...
				update_option('mo_o365_is_new_user',1);
                $uid = get_current_user_id();
                $array_dissmised_pointers = explode( ',', (string) get_user_meta( $uid, 'dismissed_wp_pointers', TRUE ) );
                $array_dissmised_pointers = array_merge($array_dissmised_pointers,Mo_o365_options_enum_pointers::$DEFAULT);
                $array_dissmised_pointers = array_merge($array_dissmised_pointers,Mo_o365_options_enum_pointers::$DEFAULT_SKIP);
		...
		update_option('my_plugin_do_activation_redirect', true);
		$this->enable_guest();
	}

	static function mo_o365_check_option_admin_referer($option_name){
	...
	function miniorange_login_widget_saml_save_settings() {
		...
		if (get_option('my_plugin_do_activation_redirect')) {
			delete_option('my_plugin_do_activation_redirect');
			
			if(!isset($_GET['activate-multi']))
				{
					wp_redirect(admin_url() . 'admin.php?page=mo_o365_saml_settings');
					exit;
				}
		}
		if ( current_user_can( 'manage_options' ) ) {

		    if(self::mo_o365_check_option_admin_referer('dismiss_pointers'))
            {
                $uid = get_current_user_id();
                $array_dissmised_pointers = explode( ',', (string) get_user_meta( $uid, 'dismissed_wp_pointers', TRUE ) );
                if ( isset( $_GET['tab'] ) ) {
                ...
                if($active_tab == 'save')
                    $array_dissmised_pointers=array_merge($array_dissmised_pointers,Mo_o365_options_enum_pointers::$SERVICE_PROVIDER);
                elseif($active_tab == 'config')
                    $array_dissmised_pointers=array_merge($array_dissmised_pointers,Mo_o365_options_enum_pointers::$IDENTITY_PROVIDER);
                elseif ($active_tab == 'opt')
                    $array_dissmised_pointers=array_merge($array_dissmised_pointers,Mo_o365_options_enum_pointers::$ATTRIBUTE_MAPPING);
                elseif ($active_tab == 'general')
                    $array_dissmised_pointers=array_merge($array_dissmised_pointers,Mo_o365_options_enum_pointers::$REDIRECTION_LINK);
                update_user_meta($uid,'dismissed_wp_pointers',implode(",",$array_dissmised_pointers));
                return;
            ...
            if(self::mo_o365_check_option_admin_referer('restart_plugin_tour'))
            {

                $uid = get_current_user_id();
                $array_dissmised_pointers = explode( ',', (string) get_user_meta( $uid, 'dismissed_wp_pointers', TRUE ) );
                $array_dissmised_pointers=array_diff($array_dissmised_pointers,Mo_o365_options_enum_pointers::$DEFAULT);
                update_user_meta($uid,'dismissed_wp_pointers',implode(",",$array_dissmised_pointers));
            ...
            if(self::mo_o365_check_option_admin_referer('skip_plugin_tour'))
            {
		    ...
		    if(self::mo_o365_check_option_admin_referer("clear_attrs_list")){
		        delete_option("mo_saml_test_config_attrs");
		        update_option('mo_saml_message','List of attributes cleared');
		        $this->mo_o365_saml_show_success_message();
            }

            if ( self::mo_o365_check_option_admin_referer("clear_pointers")) {
                $uid = get_current_user_id();
                $array_dissmised_pointers = explode( ',', (string) get_user_meta( $uid, 'dismissed_wp_pointers', TRUE ) );
                if ( isset( $_GET['tab'] ) ) {
                    $active_tab = $_GET['tab'];
                ...
                if($active_tab == 'save')
                    $array_dissmised_pointers = array_diff($array_dissmised_pointers,Mo_o365_options_enum_pointers::$SERVICE_PROVIDER);
                elseif($active_tab == 'config')
                    $array_dissmised_pointers = array_diff($array_dissmised_pointers,Mo_o365_options_enum_pointers::$IDENTITY_PROVIDER);
                elseif ($active_tab == 'opt')
                    $array_dissmised_pointers = array_diff($array_dissmised_pointers,Mo_o365_options_enum_pointers::$ATTRIBUTE_MAPPING);
                elseif ($active_tab == 'general')
                    $array_dissmised_pointers = array_diff($array_dissmised_pointers,Mo_o365_options_enum_pointers::$REDIRECTION_LINK);
            ...
            if( self::mo_o365_check_option_admin_referer("change_miniorange")){
                self::mo_o365_saml_remove_account();
                update_option('mo_saml_guest_enabled',true);
                
                return;
            }
			if ( self::mo_o365_check_option_admin_referer("login_widget_saml_save_settings")) {
				if ( ! mo_o365_saml_is_curl_installed() ) {
					...
					$this->mo_o365_saml_show_error_message();
				...
				if ( $this->mo_o365_saml_check_empty_or_null( $_POST['saml_identity_name'] ) || $this->mo_o365_saml_check_empty_or_null( $_POST['saml_login_url'] ) || $this->mo_o365_saml_check_empty_or_null( $_POST['saml_issuer'] ) ) {
				...
				foreach ( $saml_x509_certificate as $key => $value ) {
					if ( empty( $value ) ) {
						...
						$saml_x509_certificate[ $key ] = Mo_o365_Utilities::sanitize_certificate( $value );
			...
			if(self::mo_o365_check_option_admin_referer('mo_saml_update_idp_settings_option')){
			...
			if (self::mo_o365_check_option_admin_referer("login_widget_saml_attribute_mapping") ) {
			...
			if (self::mo_o365_check_option_admin_referer("login_widget_saml_role_mapping")) {
			...
			if(self::mo_o365_check_option_admin_referer("mo_saml_demo_request_option")){
			...
			if (self::mo_o365_check_option_admin_referer("saml_upload_metadata")) {
				if ( ! function_exists( 'wp_handle_upload' ) ) {
					require_once( ABSPATH . 'wp-admin/includes/file.php' );
				}
				$this->_handle_upload_metadata();
			}
			if ( self::mo_o365_check_option_admin_referer("mo_saml_register_customer")) {
				...
				if ( $this->mo_o365_saml_check_empty_or_null( $_POST['email'] ) || $this->mo_o365_saml_check_empty_or_null( $_POST['password'] ) || $this->mo_o365_saml_check_empty_or_null( $_POST['confirmPassword'] ) ) {
					...
					if(!is_null($content)){
						if ( strcasecmp( $content['status'], 'CUSTOMER_NOT_FOUND' ) == 0 ) {
							...
							if(is_array($response) && array_key_exists('status', $response) && $response['status'] == 'success'){
								wp_redirect( admin_url( '/admin.php?page=mo_o365_saml_settings&tab=licensing' ), 301 );
								exit;
							}
						} else {
            ...
            else if( self::mo_o365_check_option_admin_referer("mosaml_metadata_download")){
                mo_o365_generate_metadata(true);
            }
			if ( self::mo_o365_check_option_admin_referer("mo_saml_verify_customer") ) {    
				...
				$email    = '';
				$password = '';
				if ( $this->mo_o365_saml_check_empty_or_null( $_POST['email'] ) || $this->mo_o365_saml_check_empty_or_null( $_POST['password'] ) ) {
					update_option( 'mo_saml_message', 'All the fields are required. Please enter valid entries.' );
					$this->mo_o365_saml_show_error_message();
			...
			else if ( self::mo_o365_check_option_admin_referer("mo_saml_contact_us_query_option") ) {
				...
				if(array_key_exists('send_plugin_config',$_POST)===true) {
                    $plugin_config_json = mo_o365_import_export(true, true);
				...
				$customer = new Mo_o365_Customersaml();
				if ( $this->mo_o365_saml_check_empty_or_null( $email ) || $this->mo_o365_saml_check_empty_or_null( $query ) ) {
			...
			else if ( self::mo_o365_check_option_admin_referer("mo_saml_go_back") ) {
			...
			else if ( self::mo_o365_check_option_admin_referer("mo_saml_goto_login") ) {
				delete_option( 'mo_saml_new_registration' );
				update_option( 'mo_saml_verify_customer', 'true' );
			}
			else if ( self::mo_o365_check_option_admin_referer("mo_saml_forgot_password_form_option") ) {
				if ( ! mo_o365_saml_is_curl_installed() ) {
				...
				$customer = new Mo_o365_Customersaml();
				$content  = json_decode( $customer->mo_o365_saml_forgot_password( $email ), true );
				if(!is_null($content)){
			...
			if ( self::mo_o365_check_option_admin_referer("mo_skip_feedback") ) {
			...
			if ( self::mo_o365_check_option_admin_referer("mo_feedback") ) {
				$user = wp_get_current_user();
				...
				if ( isset( $deactivate_reason_message ) ) {
					$deactivate_reason_message =  $deactivate_reason_message;
				}else{
				...
				if(isset($_POST['get_reply']))
					$get_reply = htmlspecialchars($_POST['get_reply']);
				if(empty($get_reply)){
					$get_reply = "don't reply";
				}else{
					$get_reply = "yes";
				}
				$message.=' [Reply :'.$get_reply.']';
				 
				$rating="";
				if (isset($_POST['rate']))   
						$rate_v = htmlspecialchars($_POST['rate']);
				
				$message.= ', [Rating (on a scale from 1 to 5):'.$rate_v.']';
				...
				$feedback_reasons = new Mo_o365_Customersaml();
            ...
            if ( self::mo_o365_check_option_admin_referer("molicensingplanselection") ) {
	...
	function mo_o365_saml_show_error_message() {
		remove_action( 'admin_notices', array( $this, 'mo_o365_saml_error_message' ) );
		add_action( 'admin_notices', array( $this, 'mo_o365_saml_success_message' ) );
	}

	public function mo_o365_saml_check_empty_or_null( $value ) {
	...
	private function mo_o365_saml_show_success_message() {
		remove_action( 'admin_notices', array( $this, 'mo_o365_saml_success_message' ) );
		add_action( 'admin_notices', array( $this, 'mo_o365_saml_error_message' ) );
	}
	...
	function _handle_upload_metadata() {
		if ( isset( $_FILES['metadata_file'] ) || isset( $_POST['metadata_url'] ) ) {
			if ( ! empty( $_FILES['metadata_file']['tmp_name'] ) ) {
				...
				$response = Mo_o365_Utilities::mo_o365_saml_wp_remote_get($url);
				if(!is_null($response))
					$file = $response['body'];
				else
					$file = null;
	...
	function upload_metadata( $file ) {
		...
		if ( ! empty( $first_child ) ) {
			$metadata           = new Mo_o365_IDPMetadataReader( $document );
	...
	function mo_o365_sso_menu() {
		
        $slug = 'mo_o365_saml_settings';
		add_menu_page( 'MO SAML Settings ' . __( 'Configure SAML Identity Provider for SSO', 'mo_o365_saml_settings' ), 'WordPress SSO - Office 365 / Azure Login', 'administrator', $slug, array(
			$this,
			'mo_o365_login_widget_saml_options'
		), plugin_dir_url( __FILE__ ) . 'images/miniorange.png' );
        add_submenu_page( $slug	,'WordPress SSO - Office 365 / Azure Login'	,'Plugin Configuration','manage_options','mo_o365_saml_settings'
            , array( $this, 'mo_o365_login_widget_saml_options'));
        add_submenu_page( $slug	,'WordPress SSO - Office 365 / Azure Login'	,'Premium Plans','manage_options','mo_saml_licensing'
            , array( $this, 'mo_o365_login_widget_saml_options'));
	}

	function mo_o365_saml_authenticate() {
		$redirect_to = '';
		...
		if ( get_option( 'mo_saml_enable_login_redirect' ) == 'true' ) {
			...
			$this->mo_o365_saml_redirect_for_authentication( $redirect_to );
		}
	}

	function mo_o365_saml_redirect_for_authentication( $relay_state ) {

			if ( mo_o365_saml_is_sp_configured() && ! is_user_logged_in() ) {
				...
				$samlRequest    = Mo_o365_Utilities::createAuthnRequest( $acsUrl, $issuer, $force_authn );
				$redirect       = $ssoUrl;
	...
	function mo_o365_saml_auto_redirect() {
		...
		if ( get_option( 'mo_saml_registered_only_access' ) == 'true' ) {
			...
			$relay_state = mo_o365_saml_get_current_page_url();

			$this->mo_o365_saml_redirect_for_authentication( $relay_state );
		}
	}

	function mo_o365_saml_modify_login_form() {
		echo '<input type="hidden" name="saml_sso" value="false">' . "\n";
	}

	function mo_o365_get_saml_shortcode() {
		if ( ! is_user_logged_in() ) {
    ...
    function mo_o365_saml_plugin_action_links( $links ) {
        $links = array_merge( array(
            '<a href="' . esc_url( admin_url( 'admin.php?page=mo_o365_saml_settings' ) ) . '">' . __( 'Settings', 'textdomain' ) . '</a>'
        ), $links );
        return $links;
    }
